...
import { buildQuery, BaseQueryBuilder } from "../../../src/query2/builder/index.js"
...
const createIssueInput = (graph: D2) => graph.newInput<[number, Record<string, unknown>]>()
const createUserInput = (graph: D2) => graph.newInput<[number, Record<string, unknown>]>()
...
const sendIssueData = (input: any, issues: Issue[]) => {
  input.sendData(
    new MultiSet(issues.map((issue) => [[issue.id, issue as unknown as Record<string, unknown>], 1]))
  )
}

const sendUserData = (input: any, users: User[]) => {
  input.sendData(
    new MultiSet(users.map((user) => [[user.id, user as unknown as Record<string, unknown>], 1]))
  )
}
  ...
  describe("Subqueries in JOIN clause", () => {
    ...
    it("compiles and executes subquery in join clause", () => {
      ...
      results.forEach((result) => {
        ...
        if (result.userName) { // Only check defined userNames
