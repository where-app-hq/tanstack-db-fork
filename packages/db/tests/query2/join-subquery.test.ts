...
const sampleIssues: Array<Issue> = [
  ...
  { id: 5, title: "Feature 2", status: "in_progress", projectId: 2, userId: 2, duration: 15, createdAt: "2024-01-05" },
]

const sampleUsers: Array<User> = [
  { id: 1, name: "Alice", status: "active", email: "alice@example.com", departmentId: 1 },
  { id: 2, name: "Bob", status: "active", email: "bob@example.com", departmentId: 1 },
  { id: 3, name: "Charlie", status: "inactive", email: "charlie@example.com", departmentId: 2 },
  { id: 4, name: "Dave", status: "active", email: "dave@example.com", departmentId: undefined },
]
...
describe(`Join with Subqueries`, () => {
  describe(`subqueries in FROM clause with joins`, () => {
    ...
    test(`should join collection with subquery - left join`, () => {
      ...
      const activeUserIssues = results.filter(r => r.user_name !== undefined)
      ...
      const joinQuery = createLiveQueryCollection({
        query: (q) => {
          ...
          return q
            ...
            .join(
              { activeUser: activeUsers },
              ({ longIssue, activeUser }) => eq(longIssue.userId, activeUser.id),
              'inner'
            )
