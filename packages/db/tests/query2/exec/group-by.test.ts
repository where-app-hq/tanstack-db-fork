...
const sampleCustomers: Array<Customer> = [
  { id: 1, name: "Alice Corp", segment: "enterprise", region: "north", active: true },
  { id: 2, name: "Bob Inc", segment: "mid-market", region: "south", active: true },
  { id: 3, name: "Charlie LLC", segment: "small", region: "north", active: false },
]
...
function createCustomersCollection() {
  return createCollection(
    mockSyncCollectionOptions<Customer>({
      id: "test-customers",
      getKey: (customer) => customer.id,
      initialData: sampleCustomers,
    })
  )
}
...
describe("Query GROUP BY Execution", () => {
  ...
  describe("Multiple Column Grouping", () => {
    ...
    test("group by status and product_category", async () => {
      ...
      const completedElectronics = statusCategorySummary.get('["completed","electronics"]')
  ...
  // TODO: Add HAVING clause tests when HAVING is properly implemented in query2
  ...
  describe("Live Updates with GROUP BY", () => {
    ...
    test("live updates when inserting new orders", async () => {
      ...
      ordersCollection.utils.begin()
      ordersCollection.utils.write({ type: "insert", value: newCustomerOrder })
      ordersCollection.utils.commit()
    ...
    test("live updates when updating existing orders", async () => {
      ...
      const updatedOrder = { ...sampleOrders.find(o => o.id === 3)!, status: "completed" }
    ...
    test("live updates when deleting orders", async () => {
      ...
      ordersCollection.utils.begin()
      ordersCollection.utils.write({ type: "delete", value: lastOrderToDelete })
      ordersCollection.utils.commit()
